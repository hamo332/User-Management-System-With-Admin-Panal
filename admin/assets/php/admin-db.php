...
class Admin extends Database
{
    ...
    {
        $sql = "SELECT gender, COUNT(*) AS number FROM users WHERE gender !=''
        GROUP BY gender";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
    ...
    {
        $sql = "SELECT notes.id, notes.title, notes.note, notes.creat_at,
        notes.updated_at, users.name
    ...
    public function fetchFeedback()
    {
        ...
        ,feedback.created_at, feedback.uid
        ,users.name, users.email FROM feedback
        INNER JOIN users ON feedback.uid = users.id WHERE replied != 1
    ...
    ,users.name
    ,users.email
    FROM notification INNER JOIN users
    ON notification.uid = users.id
    WHERE type = 'admin'
